...
public class MenuFragment extends Fragment {
    private final  int[] BOTONES_MENU = {R.id.ibCartas, R.id.ibCarrito, R.id.ibUbicanos, R.id.ibConfiguracion};
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        //crear los eventos onClick y asociar a los botones
        ImageButton imgButon;
        for( int i=0;i< BOTONES_MENU.length;i++){
            imgButon = vista.findViewById(BOTONES_MENU[i]);
            final int BOTON_ELEGIDO = i;
            imgButon.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    Activity activity = getActivity();
                    ((Menu)activity).menu(BOTON_ELEGIDO);
                }
            });
        }
